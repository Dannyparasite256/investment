...
const messages = computed<TicketMessage[]>(() => activeTicket.value?.messages || [])
...
async function openTicket(id: string, replace = false) {
  ...
  try {
    ...
    // Keep sidebar in sync with latest messages
...
async function reply() {
  if (!body.value.trim() || !activeTicket.value || !canReply.value) return
  sending.value = true
  ...
  try {
    ...
    activeTicket.value.messages.push(msg)
    ...
    if (idx >= 0) {
      tickets.value[idx] = {
        ...tickets.value[idx],
        messages: activeTicket.value.messages,
    ...
    <section class="wa-pane">
      <template v-if="activeTicket">
        <header class="wa-chat-head">
          <Button
            ...
            @click="closeChat"
          />
          <span class="wa-avatar support lg">S</span>
          <div class="wa-chat-meta">
            <h2 class="truncate">{{ activeTicket.subject }}</h2>
            <p class="muted">
              Support · {{ activeTicket.category }}
              <Tag
                :value="activeTicket.status"
                :severity="statusSeverity(activeTicket.status)"
                class="wa-status-inline"
              />
            </p>
          </div>
        </header>
        ...
        <div ref="messagesEl" class="wa-messages" :class="{ loading: loadingChat }">
          <div class="wa-day-pill">Conversation started</div>
          <div
            v-for="m in messages"
            :key="m.id"
            class="wa-bubble-row"
          ...
          >
            <div class="wa-bubble">
              ...
              <div class="wa-meta">
                ...
                <i v-if="!m.is_staff_reply" class="pi pi-check-circle" title="Sent" />
              </div>
            </div>
          </div>
          <div v-if="!messages.length && !loadingChat" class="wa-empty-msgs muted">
      ...
      <div v-else class="wa-placeholder">
        <div class="wa-placeholder-card">
          ...
          <p class="muted">
            Select a conversation on the left, or start a new chat about deposits, withdrawals, KYC, or investments.
          </p>
...
.wa-pane {
  ...
  background:
    ...
    radial-gradient(circle at 80% 90%, rgba(18, 140, 126, 0.08), transparent 45%);
}
.wa-chat-head {
  display: flex;
  align-items: center;
...
.wa-status-inline { transform: scale(0.8); }
...
.wa-meta i { font-size: 0.7rem; color: #53bdeb; }
.wa-empty-msgs {
  text-align: center;
  margin: auto;
  padding: 2rem;
